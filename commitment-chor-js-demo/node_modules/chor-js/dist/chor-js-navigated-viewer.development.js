...
function SelectionBehavior(eventBus, selection, canvas, elementRegistry) {
  ...
  eventBus.on('element.click', function (event) {
    ...
    var add = (0, _Mouse.hasPrimaryModifier)(event); // select OR deselect element in multi selection
...
exports.isInitiating = isInitiating;
exports.getBandHeight = getBandHeight;
exports.hasBandMarker = hasBandMarker;
exports.getBandGapIndex = getBandGapIndex;
exports.getBandKind = getBandKind;
exports.updateParticipantBandKinds = updateParticipantBandKinds;
exports.resetAllBands = resetAllBands;
exports.updateBands = updateBands;
exports.heightOfTopBands = heightOfTopBands;
exports.heightOfBottomBands = heightOfBottomBands;
...
function updateParticipantBandKinds(activity, diBands) {
...
function resetAllBands(activity, diBands, bounds) {
  // set the bounds (except for y) for each band
  diBands.forEach(diBand => {
    diBand.bounds = (0, _minDash.assign)(diBand.bounds, {
      x: bounds.x,
      width: bounds.width,
  ...
  updateParticipantBandKinds(activity, diBands); // messages can only be visible for choreography tasks

  if (!(0, _ModelUtil.is)(activity, 'bpmn:ChoreographyTask')) {
    diBands.forEach(diBand => {
...
function updateBands(injector, activityShape) {
...
function heightOfTopBands(activityShape) {
...
function heightOfBottomBands(activityShape) {
  // Calculate using the participants directly because when this method is called
  // the participant bands might not have been created yet.
  let participants = activityShape.businessObject.participantRef || [];
  let totalHeight = 0;
...
exports.removeAndGetIndex = removeAndGetIndex;
exports.getMessageShape = getMessageShape;
exports.getMessageFlow = getMessageFlow;
exports.linkMessageFlowSemantics = linkMessageFlowSemantics;
exports.unlinkMessageFlowSemantics = unlinkMessageFlowSemantics;
exports.createMessageFlowSemantics = createMessageFlowSemantics;
exports.createMessageFlow = createMessageFlow;
exports.createMessageShape = createMessageShape;
exports.getAttachedMessageShape = getAttachedMessageShape;
exports.getAttachedMessageBounds = getAttachedMessageBounds;
exports.MESSAGE_DISTANCE = void 0;
...
const MESSAGE_DISTANCE = 20;
exports.MESSAGE_DISTANCE = MESSAGE_DISTANCE;
...
function getMessageShape(bandShape) {
  return bandShape.children && bandShape.children.find(child => (0, _ModelUtil.is)(child, 'bpmn:Message'));
}
/**
...
function getMessageFlow(task, message) {
...
function linkMessageFlowSemantics(injector, task, messageFlow, indices) {
...
function unlinkMessageFlowSemantics(injector, task, messageFlow) {
  let canvas = injector.get('canvas');
  let choreo = canvas.getRootElement().businessObject;
  let definitions = choreo.$parent;
  let message = messageFlow.messageRef;
  let indices = {}; // unlink the message from definitions

  indices.definitionsIndex = removeAndGetIndex(definitions.get('rootElements'), message); // unlink the message flow from the choreography

  indices.choreoIndex = removeAndGetIndex(choreo.get('messageFlows'), messageFlow); // unlink the message flow from the task (oldParent.parent)

  indices.taskIndex = removeAndGetIndex(task.get('messageFlowRef'), messageFlow);
  return indices;
}
/**
 * @param {Object} injector
...
function createMessageFlowSemantics(injector, task, participant) {
  let moddle = injector.get('moddle'); // create the message object
...
function createMessageFlow(injector, message, sourceParticipant, targetParticipant) {
...
function createMessageShape(injector, bandShape, messageFlow) {
  ...
  (0, _minDash.assign)(data, getAttachedMessageBounds(bandShape));
...
function getAttachedMessageShape(bandShape) {
  return bandShape.children.find(m => (0, _ModelUtil.is)(m, 'bpmn:Message'));
}
/**
...
function getAttachedMessageBounds(bandShape) {
...
exports.default = ChoreoRenderer;
...
var _BandUtil = require("../util/BandUtil");
...
function ChoreoRenderer(config, eventBus, textRenderer, pathMap) {
  // Because of the priority this does not work with the injector
  _BaseRenderer.default.call(this, eventBus, 2000); // Convenience functions to get the first color from a prioritized list.
  ...
  this.drawMessage = function (p, element) {
    ...
    let isInitiating = !bandKind.endsWith('non_initiating'); // First, draw the connecting dotted line

    let connector = (0, _tinySvg.create)('path');
    (0, _tinySvg.attr)(connector, {
      d: (0, _RenderUtil.componentsToPath)([['M', element.width / 2, isBottom ? -_MessageUtil.MESSAGE_DISTANCE : element.height], ['l', 0, _MessageUtil.MESSAGE_DISTANCE]]),
      stroke: getStrokeColor(element.parent.diBand),
      strokeWidth: 2,
      strokeDasharray: '0, 4',
      strokeLinecap: 'round'
    });
    (0, _tinySvg.append)(p, connector); // Then, draw the envelope

    let envelope = (0, _tinySvg.create)('path');
    ...
    (0, _tinySvg.append)(p, envelope); // Then, attach the label
  ...
  this.drawParticipantBand = function (p, element) {
    ...
    const needsAdditionalOutline = bandStroke || activityStroke; // Draw the participant band background if necessary, that is, if the band is
    // non-initiating or has a custom fill

    if (!isInitiating || needsSolidFill) {
      let bandShape = (0, _tinySvg.create)('path');
      (0, _tinySvg.attr)(bandShape, {
      ...
      (0, _tinySvg.append)(p, bandShape);
    }

    attachMarkerToParticipant(p, element); // If we have a custom fill or stroke, we have to again place a stroke around the
    // band to fix rendering artifacts around the task stroke

    if (needsSolidFill || needsAdditionalOutline) {
      let bandOutline = (0, _tinySvg.create)('path');
      (0, _tinySvg.attr)(bandOutline, {
        d: getParticipantBandOutline(0, 0, element.width, element.height, bandKind),
        fill: 'none',
        stroke: getStrokeColor(element.diBand, getStrokeColor(element.parent.businessObject.di)),
        strokeWidth: 2
      });
      (0, _tinySvg.append)(p, bandOutline);
    } else {
      // Otherwise, a line below or above the band is enough
      if (isTop || isMiddle) {
        let line = (0, _tinySvg.create)('path');
        (0, _tinySvg.attr)(line, {
          d: (0, _RenderUtil.componentsToPath)([['M', 0, element.height], ['l', element.width, 0]]),
          stroke: getStrokeColor(element.parent.businessObject.di),
          strokeWidth: 2
        });
        (0, _tinySvg.append)(p, line);
      }

      if (isBottom || isMiddle) {
        let line = (0, _tinySvg.create)('path');
        (0, _tinySvg.attr)(line, {
          d: (0, _RenderUtil.componentsToPath)([['M', 0, 0], ['l', element.width, 0]]),
          stroke: getStrokeColor(element.parent.businessObject.di),
          strokeWidth: 2
        });
        (0, _tinySvg.append)(p, line);
    ...
    let label = getBoxedLabel(element.businessObject.name, {
      ...
      y: 0,
      width: element.width,
      height: element.height - ((0, _BandUtil.hasBandMarker)(element.businessObject) ? MARKER_HEIGHT : 0)
    }, 'center-middle');
    (0, _tinySvg.append)(p, label);
    return p;
  ...
  this.drawChoreographyActivity = function (p, element) {
    ...
    let shape = (0, _tinySvg.create)('path');
    (0, _tinySvg.attr)(shape, {
      d: getTaskOutline(0, 0, element.width, element.height, (0, _ModelUtil.is)(element, 'bpmn:CallChoreography') ? 2 : 0),
      fill: getFillColor(element.businessObject.di),
      fillOpacity: DEFAULT_FILL_OPACITY,
      stroke: getStrokeColor(element.businessObject.di),
      strokeWidth: (0, _ModelUtil.is)(element, 'bpmn:CallChoreography') ? 6 : 2
    });
    (0, _tinySvg.append)(p, shape); // Attach markers

    let hasMarkers = attachMarkerToChoreoActivity(p, element); // Place the label correctly taking into account whether we have markers

    let top = (0, _BandUtil.heightOfTopBands)(element);
    let bottom = element.height - (0, _BandUtil.heightOfBottomBands)(element) - (hasMarkers ? 20 : 0);
    let align = 'center-middle';

    if (((0, _ModelUtil.is)(element, 'bpmn:SubChoreography') || (0, _ModelUtil.is)(element, 'bpmn:CallChoreography')) && !element.collapsed) {
      align = 'left';
    }
  ...
  function attachMarkerToChoreoActivity(parentGfx, element) {
    ...
    const bottomBandHeight = (0, _BandUtil.heightOfBottomBands)(element);
    ...
    let offset = isCollapsed && hasLoopMarker ? 10 : 0; // Draw sub choreography marker

    if (isCollapsed) {
      (0, _SvgTransformUtil.translate)(drawRect(parentGfx, 14, 14, (0, _minDash.assign)({
        strokeWidth: 2
  ...
  function attachMarkerToParticipant(parentGfx, element) {
    ...
    if ((0, _BandUtil.hasBandMarker)(element.businessObject)) {
  ...
  function drawMarker(type, parentGfx, d, attrs) {
    ...
    const path = (0, _tinySvg.create)('path');
    (0, _tinySvg.attr)(path, {
      d: d
    });
    (0, _tinySvg.attr)(path, attrs);
...
(0, _inherits.default)(ChoreoRenderer, _BaseRenderer.default);
ChoreoRenderer.$inject = ['config', 'eventBus', 'textRenderer', 'pathMap'];

ChoreoRenderer.prototype.canRender = function (element) {
  return (0, _ModelUtil.is)(element, 'bpmn:ChoreographyActivity') || (0, _ModelUtil.is)(element, 'bpmn:Participant') || (0, _ModelUtil.is)(element, 'bpmn:Message');
};

ChoreoRenderer.prototype.drawShape = function (p, element) {
  if ((0, _ModelUtil.is)(element, 'bpmn:ChoreographyActivity')) {
  ...
  } else if ((0, _ModelUtil.is)(element, 'bpmn:Message')) {
    return this.drawMessage(p, element);
  }
};

ChoreoRenderer.prototype.getShapePath = function (shape) {
  if ((0, _ModelUtil.is)(shape, 'bpmn:ChoreographyActivity')) {
    return getTaskOutline(shape.x, shape.y, shape.width, shape.height, (0, _ModelUtil.is)(shape, 'bpmn:CallChoreography') ? 6 : 1);
  } else if ((0, _ModelUtil.is)(shape, 'bpmn:Participant')) {
    return getParticipantBandOutline(shape.x, shape.y, shape.width, shape.height, shape.diBand.participantBandKind);
  } else if ((0, _ModelUtil.is)(shape, 'bpmn:Message')) {
...
function getEnvelopePath(width, height) {
  ...
  return (0, _RenderUtil.componentsToPath)(path);
}
...
function drawRect(parentGfx, width, height, attrs) {
  let rect = (0, _tinySvg.create)('rect');
  (0, _tinySvg.attr)(rect, {
    x: 0,
    y: 0,
    width: width,
...
var _draw = _interopRequireDefault(require("bpmn-js/lib/draw"));

var _ChoreoRenderer = _interopRequireDefault(require("./ChoreoRenderer"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var _default = {
  __init__: ['choreoRenderer'],
  __depends__: [_draw.default],
  choreoRenderer: ['type', _ChoreoRenderer.default]
};
exports.default = _default;
...
class ChoreoUtil {
  ...
  choreographies() {
    return this.definitions().rootElements.filter(element => (0, _ModelUtil.is)(element, 'bpmn:Choreography'));
  }
...
exports.default = ChoreoUtil;
ChoreoUtil.$inject = ['bpmnjs', 'canvas'];
...
var _ChoreoUtil = _interopRequireDefault(require("./ChoreoUtil"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var _default = {
  __init__: ['choreoUtil'],
  choreoUtil: ['type', _ChoreoUtil.default]
};
exports.default = _default;
...
exports.default = InitialRenderVisitor;

var _ModelUtil = require("bpmn-js/lib/util/ModelUtil");

var _objectRefs = _interopRequireDefault(require("object-refs"));

var _Util = require("bpmn-js/lib/import/Util");

var _minDash = require("min-dash");

var _LabelUtil = require("bpmn-js/lib/util/LabelUtil");
...
let diRefs = new _objectRefs.default({
  name: 'bpmnElement',
  enumerable: true
}, {
  name: 'di',
...
function InitialRenderVisitor(injector, eventBus, canvas, elementFactory, elementRegistry, translate, textRenderer, moddle) {
...
InitialRenderVisitor.prototype.visit = function (element, parentShape) {
  ...
  if (element.gfx) {
    throw new Error('already rendered ' + (0, _Util.elementToString)(element));
  }
...
InitialRenderVisitor.prototype._registerDi = function (di) {
  ...
  if (bpmnElement) {
    ...
    if (!(0, _ModelUtil.is)(bpmnElement, 'bpmn:Participant')) {
      if (bpmnElement.di) {
        throw new Error('multiple DI elements defined for ' + (0, _Util.elementToString)(bpmnElement));
      } else {
        diRefs.bind(bpmnElement, 'di');
        bpmnElement.di = di;
      }
    ...
    throw new Error('no bpmnElement referenced in ' + (0, _Util.elementToString)(di));
  }
};
/**
...
InitialRenderVisitor.prototype._add = function (semantic, parentShape) {
  let element;
  let isParticipantBand = (0, _ModelUtil.is)(semantic, 'bpmn:Participant') && (0, _ModelUtil.is)(parentShape, 'bpmn:ChoreographyActivity');
  let isMessageFlow = (0, _ModelUtil.is)(semantic, 'bpmn:MessageFlow');
  let isChoreoActivity = (0, _ModelUtil.is)(semantic, 'bpmn:ChoreographyActivity');
  let isChoreoTask = (0, _ModelUtil.is)(semantic, 'bpmn:ChoreographyTask');
  ...
  if (hasDI) {
    ...
    if (isChoreoActivity) {
      let participants = semantic.participantRef; // temporarily link all participant business objects to the di band
      ...
      let diBands = participants.map(participant => participant.diBand); // remove the temporary reference to the di band we stored in participants

      participants.forEach(participant => {
        delete participant.diBand;
      }); // reset all the underlying di attributes

      (0, _BandUtil.resetAllBands)(semantic, diBands, di.bounds);
    }
    /*
    ...
    if (isChoreoTask) {
      ...
      if (needsAdditionalMessageFlow) {
        let messageFlow = (0, _MessageUtil.createMessageFlowSemantics)(this._injector, semantic, semantic.initiatingParticipantRef);
        (0, _MessageUtil.linkMessageFlowSemantics)(this._injector, semantic, messageFlow);
      }
    }
    /*
    ...
    if ((0, _ModelUtil.is)(di, 'bpmndi:BPMNPlane')) {
      ...
      let collapsed = (0, _ModelUtil.is)(semantic, 'bpmn:SubChoreography') || (0, _ModelUtil.is)(semantic, 'bpmn:CallChoreography') ? !semantic.di.isExpanded : false;
      let hidden = parentShape && (parentShape.hidden || parentShape.collapsed); // the bands of a collapsed choreo activity should still be visible
      ...
      element = this._elementFactory.createShape(data); // add participant band shapes to choreo shape registry
      ...
      if ((0, _ModelUtil.is)(semantic, 'bpmn:DataStoreReference')) {
        ...
        if (!isPointInsideBBox(parentShape, (0, _LayoutUtil.getMid)(di.bounds))) {
    ...
    if (isMessageFlow) {
      ...
      if (this._elementRegistry.get(message.id)) {
         ...
         */
        let newMessage = this._moddle.create('bpmn:Message', (0, _minDash.omit)(message, ['id', '$type', 'di']));
      ...
      element = (0, _MessageUtil.createMessageShape)(this._injector, parentShape, semantic);
...
exports.default = RestoreFromCacheVisitor;
 ...
 * @param {Canvas} canvas
 */
function RestoreFromCacheVisitor(canvas) {
  this._canvas = canvas;
  this._cache = [];
}

RestoreFromCacheVisitor.$inject = ['canvas'];
...
RestoreFromCacheVisitor.prototype.visit = function (element, parentShape) {
  ...
  if (!parentShape) {
    ...
    if ((0, _ModelUtil.is)(shape, 'bpmn:SequenceFlow') || (0, _ModelUtil.is)(shape, 'bpmn:Association')) {
    ...
    if ((0, _LabelUtil.isLabelExternal)(element) && element.name) {
...
var _InitialRenderVisitor = _interopRequireDefault(require("./InitialRenderVisitor"));

var _RestoreFromCacheVisitor = _interopRequireDefault(require("./RestoreFromCacheVisitor"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var _default = {
  __init__: ['initialRenderVisitor', 'restoreFromCacheVisitor'],
  initialRenderVisitor: ['type', _InitialRenderVisitor.default],
  restoreFromCacheVisitor: ['type', _RestoreFromCacheVisitor.default]
};
exports.default = _default;
...
class ChoreoTreeWalker {
  ...
  handleParticipant(participant, parentShape) {
    ...
    if ((0, _ModelUtil.is)(parentShape, 'bpmn:ChoreographyTask')) {
  ...
  handleArtifacts(artifacts, parentShape) {
    ...
    artifacts.forEach(artifact => {
      if ((0, _ModelUtil.is)(artifact, 'bpmn:Association')) {
  ...
  handleFlowNode(flowNode, parentShape) {
    ...
    if ((0, _ModelUtil.is)(flowNode, 'bpmn:SubChoreography')) {
      this.handleSubChoreography(flowNode, flowNodeShape || parentShape);
    }

    if ((0, _ModelUtil.is)(flowNode, 'bpmn:ChoreographyActivity')) {
  ...
  handleFlowElements(flowElements, parentShape) {
    ...
    flowElements.forEach(flowElement => {
      if ((0, _ModelUtil.is)(flowElement, 'bpmn:SequenceFlow')) {
        self._deferred.push(function () {
          self.handleSequenceFlow(flowElement, parentShape);
        });
      } else if ((0, _ModelUtil.is)(flowElement, 'bpmn:BoundaryEvent')) {
        self._deferred.unshift(function () {
          self.handleBoundaryEvent(flowElement, parentShape);
        });
      } else if ((0, _ModelUtil.is)(flowElement, 'bpmn:FlowNode')) {
        self.handleFlowNode(flowElement, parentShape);
      } else {
        throw new Error('unrecognized flowElement ' + (0, _Util.elementToString)(flowElement) + ' in context ' + (parentShape ? (0, _Util.elementToString)(parentShape.businessObject) : 'null'));
      }
    });
  }
...
exports.default = ChoreoTreeWalker;
...
var _ChoreoTreeWalker = _interopRequireDefault(require("./ChoreoTreeWalker"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var _default = {
  __depends__: [_visitors.default],
  treeWalker: ['type', _ChoreoTreeWalker.default]
};
exports.default = _default;
...
var _utils = _interopRequireDefault(require("../features/utils"));
...
var _default = {
  __depends__: [_draw.default, _utils.default, _import.default]
};
exports.default = _default;
...
exports.clearShapeCache = clearShapeCache;
exports.displayChoreography = displayChoreography;
let shapeCache = {};
/**
...
function displayChoreography(viewer, bpmnDiagramOrId) {
  return new Promise((resolve, reject) => {
    ...
    const elementRegistry = viewer.get('elementRegistry'); // only cache diagrams if we are using a modeler, i.e., the command stack module
    ...
    let visitor; // if we are supposed to display a specific choreography, find it
    ...
    if (useCache) {
      ...
      stackIdx = commandStack._stackIdx; // cache shapes so we do not have to rerender them later when
    ...
    walker.setVisitor(visitor); // start the walker
...
var _ImportHandler = require("./import/ImportHandler");
 ...
 */
function Viewer(options) {
  _Viewer.default.call(this, options); // hook ID collection into the viewer
...
(0, _inherits.default)(Viewer, _Viewer.default);
...
Viewer.prototype.open = function (bpmnDiagramOrId) {
  return (0, _ImportHandler.displayChoreography)(this, bpmnDiagramOrId);
};

Viewer.prototype.importXML = function (xml, bpmnDiagram) {
  (0, _ImportHandler.clearShapeCache)();
  return _Viewer.default.prototype.importXML.call(this, xml, bpmnDiagram);
};
...
Viewer.prototype._createModdle = function (options) {
  let moddle = _Viewer.default.prototype._createModdle.call(this, options); // attach ids to moddle to be able to track
...
Viewer.prototype._collectIds = function (definitions, context) {
  ...
  let ids = moddle.ids;
  let id; // remove references from previous import
...
Viewer.prototype._modules = [].concat(_Viewer.default.prototype._modules, [_core.default]);
...
NavigatedViewer.Viewer = _Viewer.default;
